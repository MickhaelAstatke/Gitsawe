...
class NowPlayingCenter {
    ...
    static func handleItemChange(item: PlaylistItem?, index: Int, count: Int) {
        ...
        if let currentItem = item {
            ...
            nowPlayingInfo[MPMediaItemPropertyArtwork] = MPMediaItemArtwork(boundsSize: CGSize(), requestHandler: { size in return UIImage(imageLiteralResourceName: currentItem.artwork)})
//            nowPlayingInfo[MPMediaItemPropertyArtwork] = MPMediaItemArtwork(image: UIImage(imageLiteralResourceName: currentItem.artwork) )
